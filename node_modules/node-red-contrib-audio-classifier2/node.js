...
module.exports = function (RED) {
    function ModelAssetExchangeServerNode(config) {
        ...
        this.predict_startTime = config.predict_startTime;
        this.predict_startTimeType = config.predict_startTimeType || 'str';
        var node = this;
        ...
        node.on('input', function (msg) {
            ...
            if (!errorFlag && node.method === 'predict') {
                ...
                predict_nodeParam = node.predict_startTime;
                predict_nodeParamType = node.predict_startTimeType;
                if (predict_nodeParamType === 'str') {
                    predict_parameters.start_time = '0';
                } else {
                    predict_parameters.start_time = '0';
                }
                ...
                if (data) {
                    ...
                    if (data.body) {
                        if (data.body.predictions && data.body.predictions.length > 0) {
                            ...
                            msg.payload = 'nothing in the body...';
                        }
    ...
    RED.nodes.registerType("audio-classifier-2", ModelAssetExchangeServerNode);
    ...
    RED.nodes.registerType("audio-classifier-2-service", ModelAssetExchangeServerServiceNode, {
        credentials: {
            temp: { type: "text" }
        }
    });
